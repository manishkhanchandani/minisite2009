...
class mod_Imagehost {
	...
	public function uploadFile($files, $post, $user_id) {
		...
		foreach($files as $file){
			...
			// creating path
			$path = DOCPATH."/imagehost/uploadDir";
			...
			$inValidExtensions = array('jpg', 'png', 'gif', 'bmp', 'jpeg');
			if(in_array($ext, $inValidExtensions)) {	
				...
				$filehostlinks .= "Image Location: <a href='".HTTPPATH."/imagehost/uploadDir/".$name."'>".$real."</a>, or Copy the following link:<br>";
				$filehostlinks .= HTTPPATH."/imagehost/uploadDir/".$name."<br><br>";
				unset($_SESSION['checksum']);
				$success = 1;
			} else {
				throw new Exception("Image type should be of type 'jpg', 'png', 'gif', 'bmp', 'jpeg'");	
			}
		}
